...
const TINTA_OSCURA = '#11162b';

export interface DecisionTinta {
  /** true = texto blanco; false = la tinta oscura del sistema (`TINTA_OSCURA`). */
  usaBlanco: boolean;
 ...
 *
 * Los dos candidatos son SIEMPRE blanco y la tinta oscura del sistema, y esto NO depende del
 * esquema de la cáscara: el texto cae sobre el acento, no sobre la superficie. La versión anterior
 * recibía el `--ink` del shell como segundo candidato y en la plantilla oscura eso lo desactivaba
 * todo — con `--ink: #eef2f8` (L≈0.87) el blanco (L=1.0) gana para CUALQUIER color de club, así que
 * un club de primario claro (#FFD400) terminaba en 1.36:1. Por eso la función ya no toma la tinta
 * de la cáscara: no hay ningún valor de shell que pueda ganarle a blanco sobre un acento oscuro ni
 * perder contra él sobre uno claro, y ofrecerlo era pedirle a la capa 2 que decida algo de la 3.
 */
export function decidirTinta(fondoHex: string): DecisionTinta {
  const rgb = hexToRgb(fondoHex);
  if (!rgb) return { usaBlanco: true, ratio: 0 };
  ...
  const conOscura = peorContraste(relativeLuminance(hexToRgb(TINTA_OSCURA)!));
  return conBlanco >= conOscura
    ? { usaBlanco: true, ratio: conBlanco }
    : { usaBlanco: false, ratio: conOscura };
}

/**
 * Texto legible sobre el color del club, listo para CSS. `#fff` si el color no es parseable.
 *
 * Devuelve el hex oscuro LITERAL, no `var(--ink)`: `--ink-on-accent` se escribe en el `<html>`, así
 * que un `var(--ink)` se resolvía siempre contra el token global aunque abajo la cáscara declarara
 * otro — o sea que el CSS decía una cosa y pintaba otra. El literal no puede mentir.
 */
export function inkOnAccent(hex: string | null | undefined): string {
  if (!hex || !hexToRgb(hex)) return '#fff';
  return decidirTinta(hex).usaBlanco ? '#fff' : TINTA_OSCURA;
}
...
function tokensPrimeNG(c: string): Record<string, string> {
  const tokens: Record<string, string> = {
    '--p-primary-color': c,
    // La tinta de contraste de PrimeNG es la misma que la del resto: cae sobre el primario del
    // componente (botón, día seleccionado del datepicker), no sobre la superficie de la cáscara.
 ...
 *
 * No recibe la tinta de la cáscara: la tinta legible sale sólo del color del club (ver
 * `decidirTinta`), así que esta función es la misma para la landing y para el panel.
 */
